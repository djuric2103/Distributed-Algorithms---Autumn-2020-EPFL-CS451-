...
class LCausalBroadcastValidation(Validation):
    # causalRelationships : dict(pid, list(pid))
    ...
    def generateConfig(self):
        ...
        config.write("{}\n".format(self.messages))
        for pid, deps in self.causalRelationships.items():
            config.write("{} ".format(pid))
            for d in deps[:-1]:
                config.write("{} ".format(d))
            if(len(deps) >= 1):
                config.write("{}\n".format(deps[-1]))
            else:
                config.write("\n")
        ''' #A process depends on all processes that have a bigger id than himself
        for i in range(1, self.processes):
            for j in range(i, self.processes):
    ...
    def checkProcess(self, pid):
        ...
        dMsgsClocks = defaultdict(lambda : [])
        bMsgsClock = []
        filename = os.path.basename(filePath)
        ...
        with open(filePath) as f:
            ...
            for lineNumber, line in enumerate(f):
                ...
                if tokens[0] == 'd':
                    ...
                    msg = int(tokens[2])
                    clock[sender] += 1 
                    # For each message copy the current clock
                    dMsgsClocks[sender].append(clock.copy())
        ...
        return True, bMsgsClock, dMsgsClocks

    @staticmethod
    def validateClock(sDep, bClock, dClock):
        for d in sDep:
            # Not enough messages delivered
            if dClock[d] < bClock[d]:
                return False
        return True

    def checkDependencies(self, bMsgsClocksAll, dMsgsClocksAll):
        # clock : dict(host, int)
        # bMsgsClocksAll : dict(sender, list(clock))
        for sender, bClocks in bMsgsClocksAll.items():
            sDep = self.causalRelationships[sender]
            for msg, bClock in enumerate(bClocks):
                # dMsgsClocksAll : dict(receiver, dict(sender, list(clock)))
                for receiver, dClocks in dMsgsClocksAll.items():
                    # check if the message was delivered by this host
                    if msg < len(dClocks[sender]):
                        valid = LCausalBroadcastValidation.validateClock(
                            sDep, bClock, dClocks[sender][msg])
                        if(not valid):
                            error = "Message {} from process {} delivered too \
                                soon by process {}\n Broadcast clock is {} and\
                                     delivery clock is {}"
                            print(error.format(msg + 1, sender, receiver, 
                                bClock, dClocks[sender][msg]))
                            return False
        return True
    ...
    def checkAll(self, continueOnError=True):
        ...
        ok = True
        for pid in range(1, self.processes+1):
            ret, bMsgsClocksAll[pid], dMsgsClocksAll[pid] = self.checkProcess(
                pid)
            if not ret:
                ok = False
        ...
        if(not self.checkDependencies(bMsgsClocksAll, dMsgsClocksAll)):
            return False
...
def generateCausalDependencies(numProcesses):
    deps = defaultdict(lambda : [])
    deps[1] = []
    for i in range(2, numProcesses + 1):
        deps[i] = range(1, i)
    return deps
    ...
    if broadcastType == "fifo":
        ...
        validation = LCausalBroadcastValidation(
            processes, messages, logsDir, 
                generateCausalDependencies(processes))
